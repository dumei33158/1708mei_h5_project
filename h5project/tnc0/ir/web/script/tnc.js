...
$TNC={
	...
	storylistInit:function(){
		...
		var cur=$list.index($("http://www.tnc.org.cn/ir/web/script/.storylist.cur"));
		var html='';
		for(var i=0;i<_l;i++){
		...
		$(".dotlist").bind("click",function(e){
			...
			var tar=$(".dotlist").index(this),
				cur=$(".dotlist").index($("http://www.tnc.org.cn/ir/web/script/.dotlist.cur"));
			var beback=(tar<cur);
			...
			$("http://www.tnc.org.cn/ir/web/script/.dotlist.cur").removeClass("cur");
			$(this).addClass("cur");
		});
	},
	...
	renderElemData:function($obj,pos){
		...
		var headhtml='<div style="float:left;width:120px;margin-right:20px;"><img src="../../../../baby.haier.com/data/attachment/album/20140126195425.jpg"/*tpa=http://baby.haier.com/data/attachment/album/20140126195425.jpg*/ width="100%"/></div>';
		var mp3html='<div class="mp3cover" style="height:32px;padding:5px;cursor:pointer;"><img src="../img/icons/icon3.png"/*tpa=http://www.tnc.org.cn/ir/web/img/icons/icon3.png*/ /></div>';
		...
		var cover2='<div style="clear:both;height:0px;"></div><div style="position:absolute;width:28px;height:15px;left:50%;margin-left:-14px;bottom:-15px;"><img src="../img/icons/icon4.png"/*tpa=http://www.tnc.org.cn/ir/web/img/icons/icon4.png*/ /></div></div>';
		if(!!data.head){
			cover1+=('<div style="float:left;width:120px;margin-right:20px;"><img src="'+data.head+'" width="100%"/></div>');
		}
		if(!!data.audio){
			cover1+=('<div class="mp3cover" style="height:32px;padding:5px;cursor:pointer;"><img src="../img/icons/icon3.png"/*tpa=http://www.tnc.org.cn/ir/web/img/icons/icon3.png*/ />'+
			...
			if(!!audio){
				if($(this).hasClass("beplay")){
					if(typeof(audio.pause)=='function'){
						...
						$(this).find("img").attr("src",$(this).find("img").attr("src").replace(/icon7.png$/,"icon3.png"/*tpa=http://www.tnc.org.cn/ir/web/script/icon3.png*/));
					}
				}else{
					if(typeof(audio.play)=='function'){
						audio.play();
						$(this).addClass("beplay");
						$(this).find("img").attr("src",$(this).find("img").attr("src").replace(/icon3.png$/,"icon7.png"/*tpa=http://www.tnc.org.cn/ir/web/script/icon7.png*/));
					}
				}
			}
		});
	...
	pageDataInit:function(){
		var _=this;
		$.ajax({
			url:'/ivory/web/init',////////'http://www.tnc.org.cn/ir/web/script/test.json',
			data:{},
			type:"POST",
			dataType:"json",
			success:function(r){
	...
	searchVoice:function(id,callback){
		var _=this;
		$.ajax({
			url:'/ivory/web/getVoice',//,'http://www.tnc.org.cn/ir/web/script/test.json',
			data:{"id":id},
			type:"POST",
			dataType:"json",
			success:function(r){
		...
		$.ajax({
			...
			success:function(r){
				if(r.ret==0){
					...
					// 清空
					$("http://www.tnc.org.cn/ir/web/script/.namecover .username,.inputcover .isay").val('');
				}else{
					_.renderErrorTips(r.msg,2500);
				}
			},
	...
	eventInit:function(){
		...
		$(".btn_confirm").bind("click",function(e){
			var nick=$(".namecover .username").val(),
				content=$("http://www.tnc.org.cn/ir/web/script/.inputcover .isay").val();
			if($.trim(content)==""){
				//为空值
			}else{
	    ...
	    $(".btn_prev").bind("click",function(e){
	    	var year=_.getClockYear();
	    	var month=$(".mlist").index($("http://www.tnc.org.cn/ir/web/script/.mlist.cur"))+1;
